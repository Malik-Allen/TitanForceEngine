...
namespace Vulkan {
	...
	std::unique_ptr<VulkanRenderer> VulkanRenderer::graphicsInstance(nullptr);

	VulkanRenderer* VulkanRenderer::GetInstance() {
		if (graphicsInstance == nullptr) {
			graphicsInstance.reset(new VulkanRenderer);
	...
	void VulkanRenderer::OnCreate(const char* applicationName,
	...
	void VulkanRenderer::BindVertexBuffers(const std::vector<Vulkan::Buffer*>& vertexBuffers) {
	...
	void VulkanRenderer::BindIndexBuffer(Vulkan::Buffer* indexBuffer) {
		for (int i = 0; i < commandBuffers.size(); ++i) {
	...
	void VulkanRenderer::CreateVulkanInstance(const std::string& appName, const std::string& engineName, int version, bool enableValidationLayers) {
