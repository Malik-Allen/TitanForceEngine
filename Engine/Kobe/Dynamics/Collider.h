...
public:
	...
	static Vector3 GetCollisionPoint(const Ray& ray, const Sphere& sphere);
	
	// Returns true when the Ray and Box Intersect
	static bool RayAABoxCollisionDetected(const Ray& ray, const Box& box);
	// Returns the point at which the intersection takes place between a Ray and Box
	static Vector3 GetCollisionPoint(const Ray& ray, const Box& box);
	...
	static void CollisionResponse(RigidBody& body, const Plane& plane);
