...
float CMath::Quadratic(float a, float b, float c, bool returnSmallest) {
	...
	if (d < 0.0f)			// When not in debug mode and the discriminant is less than zero it will return a solution of zero
		...
		else {	// Returns the larger solution when the program prompts
			if (t1 < t2)
				return t2;
			else
				return t1;
...
bool Collider::RaySphereCollisionDetected(const Ray& ray, const Sphere& sphere) {
	...
	float c =Math::Dot(ray.origin, ray.origin) - (sphere.r * sphere.r);
...
Vector3 Collider::GetCollisionPoint(const Ray& ray, const Sphere& sphere) {
...
bool Collider::RayAABoxCollisionDetected(const Ray& ray, const Box& box) {
...
Vector3 Collider::GetCollisionPoint(const Ray& ray, const Box& box) {
	float tx, ty, tz;	// Will be the three values of t where we will check for collisions at
...
bool Collider::SpherePlaneCollisionDetected(const RigidBody& body, const Plane& plane) {
	...
	if (body.m_bodyType == dynamicBody) {
		if (Math::Distance(body.m_position, plane) - body.m_radius <= 0.0f)
			return true;
		else
			return false;
	}
...
void Collider::CollisionResponse(RigidBody& body, const Plane& plane) {
	...
	Vector3 finalVel = body.m_velocity + (2.0f * (SMath::ProjectOnPlane(-body.m_velocity, plane)));
...
bool Collider::RigidBodyCollisionDetected(const RigidBody& body1, const RigidBody& body2) {
	...
	if (body1.m_bodyType == staticBody && body2.m_bodyType == staticBody)	// When both bodies are static then there is no need to check for collisions
		...
		if (Math::Distance(body1.m_position, body2.m_position) <= (body1.m_radius + body2.m_radius))
			return true;
		else
			return false;
	}
...
void Collider::CollisionResponse(RigidBody& body1, RigidBody& body2) {
	...
	if (body1.m_bodyType != staticBody && body2.m_bodyType != staticBody) {
		...
		Vector3 collisionVelocity = (body1.m_momentum + body2.m_momentum) / (body1.m_mass + body2.m_mass);			// To determine the final velocity of the collisions we apply the conservation of momentum law
		Vector3 body1Result = (-coeffRestitution * (body1.m_velocity - body2.m_velocity)) + collisionVelocity;		// Then we use of coefficient of resitution to calculate the final velocities of the objects in this collision
		Vector3 body2Result = -((-coeffRestitution * (body1.m_velocity - body2.m_velocity)) - collisionVelocity);
		...
		if (body1.m_bodyType == staticBody) {
			...
			finalVel = body2.m_velocity + (2.0f * (Math::Project(-body2.m_velocity, diffVec)));
			body2.m_velocity = finalVel;
		}
		else if (body2.m_bodyType == staticBody) {
			diffVec = body1.m_position - body2.m_position;
			finalVel = body1.m_velocity + (2.0f * (Math::Project(-body1.m_velocity, diffVec)));
