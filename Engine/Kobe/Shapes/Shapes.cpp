...
Plane::Plane(const Vector3& v0, const Vector3& v1, const Vector3& v2) {
	Vector3 temp1 = v1 - v0;	// We can create a plane using two Vectors
	Vector3 temp2 = v2 - v1;	// We create these two intersecting Vectors by taking the differences between 3 vectors

	Vector3 normal(temp1.y * temp2.z - temp1.z * temp2.y,
...
Vector3 Plane::IntersecPoint(const Ray& r_) {
	...
	Vector3 normal(x / a, y / a, z / a);	// Normalizing this Plane's normal
...
Vector3 SMath::ProjectOnPlane(const Vector3& v, const Plane& p) {
	Plane plane(p);
	plane.normalize();
...
float SMath::Distance(const Vector3& v, const Plane& p) {
	return v.x * p.x + v.y * p.y + v.z * p.z - p.d;
}

/// Translates Plane by given Vector3
Plane SMath::Translate(const Plane& p, const Vector3& v) {
