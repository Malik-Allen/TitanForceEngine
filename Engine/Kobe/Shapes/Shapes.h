...
namespace Kobe {
	...
	struct Ray {
		...
		inline Ray(const Vector3& origin_, const Vector3& direction_) {
			origin = origin_;
	...
	struct Ray2D {
		...
		// A Ray2D constructor
		inline Ray2D(const Vector2& origin_, const Vector2& direction_) {
			origin = origin_;
			direction = direction_;
		}
		...
		// Uses the direction's unit vector
		inline Vector2 CurrentPosition(float t) {
			return origin + direction * t;
		}
	...
	struct Sphere : public Vector3 {
		...
		inline Sphere(const Vector3& v, float r_) {
	...
	public:
		...
		static Vector3 ProjectOnPlane(const Vector3& v, const Plane& p);
		...
		static Plane Translate(const Plane& p, const Vector3& v);
	};
